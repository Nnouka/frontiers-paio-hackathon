...
support to take it correctly and finish the course. Full system design lives in
[health/system.md](system.md); architecture ownership lives in [health/architecture.md](architecture.md).
...
### Team workflow
- 5 people, 5 parallel workstreams (see [health/architecture.md](architecture.md) section 6): Mobile/Frontend,
...
- Mid-build integration checkpoint against shared staging Postgres + mocked OCR/DDI responses,
  before the final end-to-end consolidation pass

## 3) Scope References

- System architecture and data flow are maintained in [health/system.md](system.md) and [health/architecture.md](architecture.md).
- Implementation backlog, epics, and user stories are maintained in [health/userstories.md](userstories.md).
- Product design specifications are maintained in [health/prd.md](prd.md).

## 4) Definition of Done

A feature is done when:
- Acceptance criteria pass
- Mobile views are verified against the target flow (search, scan, schedule, or adherence log)
...
- **Google/Firebase services only.** Frontend is Flutter; backend is Firebase Cloud Functions,
  Firestore, Cloud Storage, FCM, Cloud Scheduler/Tasks, and Vertex AI Gemini. No third-party
  backend services (no Express server, no standalone Postgres/Redis, no Twilio/SMS gateway) —
  chosen for one-console setup speed and prototyping fit within the hackathon window.
